...
ENTRY_POINT_PATTERNS = {
    # Python
    'main.py', 'app.py', 'server.py', '__main__.py', 'run.py', 'start.py',
    'manage.py', 'wsgi.py', 'asgi.py', 'gunicorn.py',  # Django/Flask patterns
    # JavaScript/TypeScript  
    'index.js', 'app.js', 'server.js', 'main.js', 'index.ts', 'app.ts', 'server.ts', 'main.ts',
    'start.js', 'start.ts', 'bootstrap.js', 'bootstrap.ts', 'entry.js', 'entry.ts',
    # Go
    'main.go', 'cmd.go', 'server.go', 'app.go', 'root.go', 'start.go',
    # Rust
    'main.rs', 'lib.rs', 'server.rs', 'app.rs', 'start.rs', 'bin.rs',
    # C/C++
    'main.c', 'main.cpp', 'main.cc', 'main.cxx', 'app.c', 'app.cpp',
    'start.c', 'start.cpp', 'entry.c', 'entry.cpp'
}

# Additional entry point path patterns (for when filename patterns fail)
ENTRY_POINT_PATH_PATTERNS = [
    'cmd/main', 'cmd/root', 'cmd/server',  # Go command patterns
    'src/main', 'src/app', 'src/server',   # Common src patterns
    'bin/main', 'bin/app', 'bin/server',   # Binary patterns
    'app/main', 'app/server', 'app/start', # App directory patterns
    'scripts/start', 'scripts/run',        # Script patterns
]

# Flexible entry point name patterns (partial matches)
ENTRY_POINT_NAME_PATTERNS = [
    'main', 'app', 'server', 'start', 'run', 'entry', 'bootstrap', 'init',
    'cmd', 'cli', 'daemon', 'service', 'worker', 'launcher'
]

# High connectivity file patterns (files likely to have many function calls)
HIGH_CONNECTIVITY_PATTERNS = {
    # General patterns
    'router', 'controller', 'service', 'handler', 'middleware',
    'api', 'core', 'engine', 'manager', 'processor', 'client',
    # Language-specific patterns
    'mod', 'module',  # Rust modules
    'pkg', 'package',  # Go packages  
    'lib', 'util', 'utils', 'helper', 'helpers',
    # Framework patterns
    'express', 'fastapi', 'gin', 'actix', 'rocket',  # Web frameworks
    'db', 'database', 'model', 'entity', 'repo', 'repository',
    # Additional patterns
    'config', 'settings', 'constants', 'types', 'interfaces',
    # Generic library patterns (added for broader coverage)
    'console', 'text', 'style', 'render', 'display', 'format',
    'parse', 'parser', 'convert', 'transform', 'process',
    'table', 'tree', 'list', 'grid', 'layout', 'widget',
    'color', 'theme', 'visual', 'graphic', 'draw', 'paint',
    'file', 'io', 'stream', 'buffer', 'cache', 'store',
    'base', 'common', 'shared', 'global', 'main', 'index'
}

# Source directory patterns across all languages
SOURCE_DIRECTORY_PATTERNS = [
    'src/', 'lib/', 'core/', 'pkg/',     # General
    'cmd/', 'internal/',                  # Go specific
    'crates/', 'modules/',                # Rust specific  
    'include/', 'source/',                # C/C++ specific
    'components/', 'services/', 'utils/'  # Framework patterns
]
...
CRITICAL_FUNCTION_NAMES = {
    'main', 'index', 'app', 'server', 'start', 'init', 'run', 'new'
}
...
def is_entry_point_file(filename: str) -> bool:
    ...
    for pattern in ENTRY_POINT_NAME_PATTERNS:
        if pattern in filename_lower and any(ext in filename_lower for ext in ['.py', '.js', '.ts', '.go', '.rs', '.c', '.cpp']):
            return True
