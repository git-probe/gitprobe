...
class SimpleExportService:
        ...
        if selection and selection.selected_nodes:
            ...
            if include_relationships:
                selected_relationships = [
                    ...
                    if (r.caller in selection.selected_nodes or 
                        r.callee in selection.selected_nodes) and r.is_resolved
                ]
            else:
                selected_relationships = []
    ...
    def export_cytoscape(
        ...
        if selection and selection.selected_nodes:
            ...
            relationships = [
                ...
                if (r.caller in selection.selected_nodes or 
                    r.callee in selection.selected_nodes)
            ]
    ...
    def export_svg(
        ...
        for i, func in enumerate(functions):
            ...
            svg_parts.append(f'<circle cx="{x}" cy="{y}" r="20" class="{node_class}" />')
            svg_parts.append(f'<text x="{x}" y="{y+4}" class="label">{display_name[:10]}</text>')
